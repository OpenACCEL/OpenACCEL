...
suite("namedvectorpass.js", function() {
    ...
    setup(function(done) {
        requirejs(["assert", "model/passes/preprocessor/namedvectorpass"], function(Assert, NamedVectorPass) {
            console.log("Loaded 'NamedVectorPass' module.");
            assert = Assert;
            namedVectorPass = new NamedVectorPass();
            done();
        });
    });
    ...
    suite("namedVectorPass", function() {
        ...
        test('parse() : a = [1 + 10, b[1 + 2], c[x:2, y:3, 5], b[0, 2, y:3, t5: 4, 6, 3, o93e: 0, 5]', function() {
            ...
            assert.deepEqual(namedVectorPass.parse(exScript, {}), resultScript);
        });

        test("replaceBrackets:y = [4, 5]", function() {
            var input = "y = [4, 5]";
            var expected = "y = {'0':4,'1': 5}";
            assert.equal(namedVectorPass.replaceBrackets(input), expected);
        });
        ...
        test('replaceBrackets() calling vector', function() {
            assert.equal(namedVectorPass.replaceBrackets('y = b[0]'), 'y = b[0]');
        });

        test('replaceBrackets() y = [1,2,[1,2]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [1,2,[1,2]]"), "y = {'0':1,'1':2,'2':{'0':1,'1':2}}");
        });
        ...
        test('replaceBrackets() calling vector in vector definition', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [1, b[0]]"), "y = {'0':1,'1': b[0]}");
        })

        test('replaceBrackets() y = [[1],[2],[3],[4],[5]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[1],[2],[3],[4],[5]]"), "y = {'0':{'0':1},'1':{'0':2},'2':{'0':3},'3':{'0':4},'4':{'0':5}}");
        })

        test('replaceBrackets() y = [[[[1]]], [[1]]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[[[1]]], [[1]]]"), "y = {'0':{'0':{'0':{'0':1}}},'1': {'0':{'0':1}}}");
        });

        test('replaceBrackets() y = [[1, a[0 + 10], b[x + 5]],[2],[3],[4],[5]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5]]"), "y = {'0':{'0':1,'1':a[0+10],'2':b[x+5]},'1':{'0':2},'2':{'0':3},'3':{'0':4},'4':{'0':5}}");
        });

        test('replaceBrackets() y = a[1 + a[0 + b[0]] + c[0]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = a[1 + a[0 + b[0]] + c[0]]"), "y = a[1 + a[0 + b[0]] + c[0]]");
        });

        test('replaceBrackets() y = a[[1,2,3]*[1,2,3]]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = a[[1,2,3]*[1,2,3]]"), "y = a[{'0':1,'1':2,'2':3}*{'0':1,'1':2,'2':3}]");
        });

        test('replaceBrackets() y = a[f([1,2,3],[1,2,3])]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = a[f([1,2,3],[1,2,3])]"), "y = a[f({'0':1,'1':2,'2':3},{'0':1,'1':2,'2':3})]");
        });

        test('replaceBrackets() y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5],f([1,2,3],[1,2,3])]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5],f([1,2,3],[1,2,3])]"), "y = {'0':{'0':1,'1':a[0+10],'2':b[x+5]},'1':{'0':2},'2':{'0':3},'3':{'0':4},'4':{'0':5},'5':f({'0':1,'1':2,'2':3},{'0':1,'1':2,'2':3})}");
        });

        test('replaceBrackets() y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5],f([a(1,2,3),2,3],[1,2,3])]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5],f([a(1,2,3),2,3],[1,2,3])]"), "y = {'0':{'0':1,'1':a[0+10],'2':b[x+5]},'1':{'0':2},'2':{'0':3},'3':{'0':4},'4':{'0':5},'5':f({'0':a(1,2,3),'1':2,'2':3},{'0':1,'1':2,'2':3})}");
        });
        test('replaceBrackets() y = [[1, a[0 + 10], b[x + 5]],[2],[3],[4],[5],f([1,2,3],[1,2,3])]', function() {
            assert.equal(namedVectorPass.replaceBrackets("y = [[1,a[0+10],b[x+5]],[2],[3],[4],[5],f([a(b(1,2,3),2,3),2,3],[1,2,3])]"), "y = {'0':{'0':1,'1':a[0+10],'2':b[x+5]},'1':{'0':2},'2':{'0':3},'3':{'0':4},'4':{'0':5},'5':f({'0':a(b(1,2,3),2,3),'1':2,'2':3},{'0':1,'1':2,'2':3})}");
        });
        /**
