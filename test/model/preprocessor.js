suite("PreProcessor", function() {
    ...
    suite("processing", function() {
        test("default settings, no units", function() {
            ...
            var expected = "(function () { exe = {}; func(x = 5)func(y = sin(exe.x()))func(z = 2  _add_  sin(exe.y()  _add_  sin(exe.x()))  _add_  4  _add_  sin(2))func(u = exe.x()  _add_  exe.y())return exe; })()"
        ...
        test("default settings, with units", function() {
            var code = "x = 5 ; kg\ny = sin(x) ; kg2.s/m\nz = 2 + sin(y + sin(x)) + 4 + sin(2)\nu = x + y";
            var output = preProcessor.process(new Script(code));
            var expected = "(function () { exe = {}; func(x = 5 ; {'kg': 1})func(y = sin(exe.x()) ; {'kg': 2, 's': 1, 'm': -1})func(z = 2  _add_  sin(exe.y()  _add_  sin(exe.x()))  _add_  4  _add_  sin(2))func(u = exe.x()  _add_  exe.y())return exe; })()"
            assert.equal(output, expected);
        });
